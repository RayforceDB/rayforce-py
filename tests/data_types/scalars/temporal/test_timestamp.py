...
def test_to_millis_unit_is_nanoseconds():
    """#M7: to_millis is (mis)named — it returns the raw engine value in
    NANOSECONDS, not milliseconds. Pin the unit so the name can't silently lie."""
    base = t.Timestamp(dt.datetime(2020, 1, 1, 0, 0, 0, tzinfo=dt.UTC))
    plus_one_second = t.Timestamp(dt.datetime(2020, 1, 1, 0, 0, 1, tzinfo=dt.UTC))
    assert plus_one_second.to_millis() - base.to_millis() == 1_000_000_000


def test_timestamp_from_int_nanos():
    """Construct from raw nanoseconds since the 2000-01-01 epoch."""
    assert t.Timestamp(800202645000000000).value == dt.datetime(
        2025, 5, 10, 14, 30, 45, tzinfo=dt.UTC
    )
