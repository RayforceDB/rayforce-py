...
# ── Quoted vs name-reference semantics (ATTR_QUOTED 0x20) ─────────────────────
# Regression for the symbol-polarity flip (#M1): a literal symbol value must be
# quoted so eval treats it as data, not as a variable-name reference.


def test_quoted_symbol_membership_resolves_as_literal():
    from rayforce import List, Operation, QuotedSymbol, Vector
    from rayforce.ffi import FFI
    from rayforce.utils.conversion import ray_to_python

    symvec = Vector(items=["a", "b", "code"], ray_type=Symbol)
    res = FFI.eval_obj(List([Operation.IN, QuotedSymbol("code"), symvec]).ptr)
    assert ray_to_python(res).value is True


def test_in_on_list_with_bare_string_does_not_recurse():
    from rayforce import List, Operation, Vector
    from rayforce.ffi import FFI

    # Must evaluate without RecursionError (literal symbol, not a variable lookup).
    l = List([Operation.IN, "asd", Vector("asd")])
    FFI.eval_obj(l.ptr)  # no exception == pass


def test_dict_symbol_key_lookup_does_not_recurse():
    from rayforce import Dict
    from rayforce.types.null import Null

    d = Dict({"code": "type", "msg": "boom"})
    assert d["code"] == "type"
    assert d["missing"] is Null
