...
class TestDictNonStringKeys:
    """Regression: non-string keys must round-trip by value, not be coerced to
    Symbol (#M2)."""

    def test_int_keys_lookup(self):
        d = t.Dict({1: "a", 2: "b", 3: "c"})
        assert d[2] == "b"
        assert d[1] == "a"

    def test_int_key_missing_returns_null(self):
    ...
    def test_int_keys_round_trip(self):
        d = t.Dict({10: "x", 20: "y"})
        assert dict(d.to_python()) == {10: "x", 20: "y"}

    def test_float_keys_lookup(self):
        d = t.Dict({1.5: "a", 2.5: "b"})
        assert d[2.5] == "b"
