...
class TestListSetItem:
    """Regression: __setitem__ must REPLACE in place, not insert/grow (#H2)."""

    def test_setitem_replaces_in_place(self):
        l = t.List(["test", 123, 555.0, True])
        l[1] = 999
        assert len(l) == 4  # length unchanged — not grown
        assert l[1].value == 999
        assert l[0].value == "test"
        assert l[2].value == 555.0
    ...
    def test_setitem_out_of_range_raises(self):
        l = t.List([1, 2, 3])
        with pytest.raises(errors.RayforceIndexError):
            l[10] = 1

    def test_setitem_negative_out_of_range_raises(self):
        l = t.List([1, 2, 3])
        with pytest.raises(errors.RayforceIndexError):
            l[-10] = 1
