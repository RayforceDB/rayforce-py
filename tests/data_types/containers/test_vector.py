...
class TestVectorToListNulls:
    """Regression: to_list must surface nulls as Null, not the raw sentinel (#M3)."""

    def test_to_list_null_is_null_not_sentinel(self):
        from rayforce.types.null import Null

        v = t.Vector(items=[1, None, 3], ray_type=t.I64)
        out = v.to_list()
        assert out[0] == 1
        assert out[1] is Null
        assert out[2] == 3

    def test_to_list_matches_to_python_with_nulls(self):
        v = t.Vector(items=[10, None, 30], ray_type=t.I32)
        to_list = v.to_list()
        to_python = [el.value if hasattr(el, "value") else el for el in v.to_python()]
        assert to_list == to_python

    def test_to_list_no_nulls_unaffected(self):
        v = t.Vector(items=[1, 2, 3], ray_type=t.I64)
        assert v.to_list() == [1, 2, 3]


class TestVectorSetItemTypePreservation:
    """Regression: __setitem__ must preserve the vector's element type (#M4)."""

    def test_setitem_int_into_i16_preserves_type(self):
        from rayforce import _rayforce_c as r

        v = t.Vector(items=[1, 2, 3], ray_type=t.I16)
        v[0] = 99
        assert r.get_obj_type(v.ptr) == r.TYPE_I16
        assert v[0].value == 99

    def test_setitem_into_f64_preserves_type(self):
        from rayforce import _rayforce_c as r

        v = t.Vector(items=[1.0, 2.0, 3.0], ray_type=t.F64)
        v[1] = 9.5
        assert r.get_obj_type(v.ptr) == r.TYPE_F64
        assert v[1].value == 9.5
