...
def test_pivot_default_aggfunc_is_min():
    """aggfunc defaults to 'min' when omitted (matches the documented default)."""
    table = Table(
        {
            "symbol": Vector(items=["AAPL", "AAPL", "GOOG"], ray_type=Symbol),
            "metric": Vector(items=["price", "price", "price"], ray_type=Symbol),
            "value": Vector(items=[150, 140, 2800], ray_type=I64),
        }
    )
    # No aggfunc passed → defaults to "min": AAPL min(150, 140) == 140.
    result = table.pivot(index="symbol", columns="metric", values="value").execute()
    assert_contains_columns(result, ["symbol", "price"])
    pv = _get_pivot_values(result, "symbol")
    assert pv["AAPL"]["price"] == 140
    assert pv["GOOG"]["price"] == 2800
