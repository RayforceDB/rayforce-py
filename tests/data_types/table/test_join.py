...
# ── Joins over reference (named/saved) tables (#M5) ──────────────────────────
# Regression: _Join must resolve a Table(name) reference to its actual table
# (evaled_ptr), not pass a quoted-literal symbol that the join can't resolve.


def test_inner_join_on_reference_tables(make_table):
    tname, _ = make_table(
        {
            "Sym": Vector(items=["AAPL", "GOOGL"], ray_type=Symbol),
            "Price": Vector(items=[100, 200], ray_type=I64),
    ...
    result = Table(tname).inner_join(Table(qname), "Sym").execute()
    assert isinstance(result, Table)
    assert_contains_columns(result, ["Sym", "Price", "Bid"])
    assert_table_shape(result, rows=2, cols=3)


def test_left_join_on_reference_tables(make_table):
    tname, _ = make_table(
        {
    ...
    result = Table(tname).left_join(Table(qname), "Sym").execute()
    assert isinstance(result, Table)
    assert_contains_columns(result, ["Sym", "Price", "Bid"])
    assert_table_shape(result, rows=2, cols=3)
