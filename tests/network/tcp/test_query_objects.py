...
def test_select_query_tcp(client, make_table):
    name, _ = make_table(
        {
...
def test_update_query_tcp(client, make_table):
    name, _ = make_table(
        {
            "id": Vector(items=["001", "002"], ray_type=Symbol),
            "age": Vector(items=[29, 34], ray_type=I64),
    ...
    result = Table(name).select("*").execute()
    assert_row(result, 0, {"id": "001", "age": 35})


def test_insert_query_tcp(client, make_table):
    name, _ = make_table(
        {
...
def test_upsert_query_tcp(client, make_table):
    name, _ = make_table(
        {
            "id": Vector(items=["001"], ray_type=Symbol),
            "age": Vector(items=[29], ray_type=I64),
...
def test_sql_select_query_tcp(client, make_table):
    name, _ = make_table(
        {
...
def test_sql_update_query_tcp(client, make_table):
    name, _ = make_table(
        {
...
def test_sql_insert_query_tcp(client, make_table):
    name, _ = make_table(
        {
    ...
    result = Table(name).select("*").execute()
    assert len(result) == 2
    assert_row(result, 1, {"id": 2})


def test_sql_upsert_query_tcp(client, make_table):
    name, _ = make_table(
        {
            "id": Vector(items=[1, 2], ray_type=I64),
            "name": Vector(items=["alice", "bob"], ray_type=Symbol),
...
def test_sql_complex_select_tcp(client, make_table):
    name, _ = make_table(
        {
