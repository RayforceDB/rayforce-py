...
def test_concat_two_tables():
    ...
    table1 = Table.from_dict({"name": ["Alice", "Bob"], "age": [30, 35]})

    table2 = Table.from_dict({"name": ["Charlie", "Dana"], "age": [40, 45]})
...
def test_inner_join_single_column():
    ...
    customers = Table.from_dict({"id": [1, 2, 3], "name": ["Alice", "Bob", "Charlie"]})

    orders = Table.from_dict(
        {"id": [2, 3, 3], "product": ["Widget", "Gadget", "Gizmo"]}
    )
...
def test_inner_join_multiple_columns():
    ...
    table1 = Table.from_dict(
        {"entity": ["A", "B", "C"], "broker": ["X", "Y", "Z"], "value1": [10, 20, 30]}
    )
...
def test_first_last_aggregations():
    ...
    result = table.group_by("group").agg(
...
def test_median_aggregation():
    ...
    table = Table.from_dict(
        {"group": ["A", "A", "A", "B", "B", "B"], "value": [10, 20, 30, 100, 200, 300]}
    )

    result = table.group_by("group").agg(median_value=table.value.median())
...
def test_lookup_dictionary():
    ...
    broker_entities = c.Dict(
        {"BROKER_1": "ENTITY_A", "BROKER_2": "ENTITY_B", "BROKER_3": "ENTITY_C"}
    )
    ...
    result = table.update(entity=lookup(broker_entities, table.broker)).execute()
