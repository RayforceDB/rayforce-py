...
def test_eval_obj_type_error_raises():
    """#L3: a TYPE_ERR result must raise, not return a raw error object."""
    from rayforce import QuotedSymbol

    with pytest.raises(errors.RayforceError):
        eval_obj(List([Operation.ADD, 1, QuotedSymbol("a")]))
