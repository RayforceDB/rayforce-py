...
class Expression:
    ...
    def _build_filtered_column(self, filtered_col: FilteredColumn) -> q.Expression:
        ...
        return q.Expression(
            p.Operation.MAP, p.Operation.AT, filtered_col.column.name, where_wrapped
        )
...
class GroupByBuilder:
    ...
    def agg(self, **aggregations) -> Table:
        ...
        query = q.SelectQuery(select_from=self._table, attributes=attributes)
...
class QueryBuilder:
    ...
    def _convert_expr(
        self, expr: Union[Expression, Column]
    ) -> Union[q.Expression, str]:
        if isinstance(expr, Expression):
...
class Table:
    ...
    def join(
        self, other: Table, on: Union[str, List[str]], how: str = "inner"
    ) -> Table:
        ...
        if how == "inner":
            ...
            result = legacy_inner_join(
                join_by=join_keys, to_join=[self_table, other_table]
            )
        else:
    ...
    def upsert(
        self, data: Union[Dict, List[Dict]], match_on: Union[str, List[str]] = "id"
    ) -> UpsertQuery:
...
class UpsertQuery:
    ...
    def execute(self) -> Table:
        ...
        result = legacy_upsert(
            query.upsert_to_ptr, query.match_ptr, query.upsertable_ptr
        )
        return Table(ptr=result.ptr)
