...
def python_to_ipc(data: t.Any) -> r.RayObject:
    ...
    if isinstance(data, InsertQuery):
        return Expression(Operation.INSERT, data.table, data.compile(ipc=True)).compile()
    if isinstance(data, UpsertQuery):
        return Expression(Operation.UPSERT, data.table, *data.compile(ipc=True)).compile()
