...
try:
    ...
    raise ImportError(
        "websockets library is required. Install it with: pip install rayforce-py[websocket]"
    ) from e


class WSServer:
    """Async WebSocket server that evaluates Rayforce IPC messages in-process.

    Each binary message is deserialized, evaluated against the in-process
    runtime (``eval_str`` for query strings, ``eval_obj`` for compiled
    objects), and the serialized result is sent back. This is a Python-side
    protocol: unlike the native TCP server, WebSocket framing is not provided
    by the C IPC layer.
    """
...
class WSServerConnection:
    ...
    async def _process_message(self, message: bytes | str) -> None:
        ...
        # Type code is signed (negative => atom). A string (atom or vector) is a
        # query to evaluate via eval_str; an error passes through; anything else
        # is a compiled object evaluated via eval_obj.
        obj_type = FFI.get_obj_type(deser)
        if obj_type == r.TYPE_ERR:
            result = deser
        elif abs(obj_type) == r.TYPE_STR:
